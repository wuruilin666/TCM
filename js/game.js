...
/* ===================== 四诊（望 / 闻 / 问 / 切） =====================
 * 四诊的「名称 / 线索分组 key / 按钮 id / 展示顺序」只在这里定义一处：
 * addClue（写线索）、setExplored（标记已探查）、renderClues（线索分组）都从它取值，
 * 避免同一套四诊名称在多处硬编码后彼此漂移。
 * tagClass 是线索的分组 key（线索区不再显示诊法标签，因此它已不对应任何 CSS 样式），
 * 也是 addClue 写入每条线索的来源标识，renderClues 靠它把线索归到对应诊法下。
...
// 舌象判断线索的摘要/详情分隔标记。inspection.js 生成内容时插入，
// renderClues 据此把「望诊·舌象」线索渲染为默认折叠（摘要 = 正确舌象 + 判断正误）。
