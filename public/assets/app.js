    ...
    const diffOrder = ['basic', 'intermediate', 'advanced'];
    // 题库筛选状态
    ...
    let inspectionNonTongueAdded = false;
    let lastAnswerWasCorrect = false;

    /* ===================== 数据与输出安全 ===================== */
    const SAFE_CASE_ID = /^[a-z]+-\d{3}$/;
    const MAX_STORED_TEXT_LENGTH = 2000;
    function escapeHtml(value) {
        return String(value ?? '').replace(/[&<>'"]/g, char => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' })[char]);
    }
    function escapeHtmlWithBreaks(value) { return escapeHtml(value).replace(/\r?\n/g, '<br>'); }
    function isNonEmptyString(value) { return typeof value === 'string' && value.trim().length > 0; }
    function isSafeCaseId(value) { return typeof value === 'string' && SAFE_CASE_ID.test(value); }
    ...
    /* ===================== 存储工具 ===================== */
    function safeGetStorage(key, fallback) { try { const data = localStorage.getItem(key); return data ? JSON.parse(data) : fallback; } catch (e) { return fallback; } }
    function safeSetStorage(key, value) { try { localStorage.setItem(key, JSON.stringify(value)); } catch (e) { console.warn('本地学习记录保存失败', e); } }
    ...
    function sanitizeStoredText(value, maxLength = MAX_STORED_TEXT_LENGTH) { return typeof value === 'string' ? value.slice(0, maxLength) : ''; }
    function getWrongCases() {
    ...
    function saveWrongCase(data) {
        if (!currentCase) return;
        const wrongs = getWrongCases().filter(w => w.id !== currentCase.id);
        wrongs.push({
            id: currentCase.id,
            title: sanitizeStoredText(currentCase.title, 200),
            chiefComplaint: sanitizeStoredText(currentCase.chiefComplaint),
    ...
    function removeWrongCase(caseId) {
    ...
    function startChallenge() { showPage('Game'); resetGameUI(); }
    function showAbout() { renderDataStats(); showPage('About'); }

    /* ===================== 闯关逻辑 ===================== */
    function resetGameUI() {
        currentDifficulty = null; currentCase = null; collectedClues = []; exploredDiags = {}; gameStarted = false;
        inquiryHistory = []; askedInquiryQuestions = []; unfinishedCases = []; currentCaseIndex = 0; inspectionNonTongueAdded = false; lastAnswerWasCorrect = false;
    ...
    function showCurrentCase() {
        if (!unfinishedCases.length) return;
        currentCase = unfinishedCases[currentCaseIndex];
        collectedClues = []; exploredDiags = { inspection: false, auscultation: false, inquiry: false, pulse: false };
        inquiryHistory = []; askedInquiryQuestions = []; gameStarted = true; inspectionNonTongueAdded = false; lastAnswerWasCorrect = false;
        document.getElementById('chiefComplaintCard').style.display = 'block';
        document.getElementById('chiefComplaintText').textContent = currentCase.chiefComplaint;
        ...
        document.getElementById('inputBasis').value = '';
        ['btnWang','btnWen','btnAsk','btnPulse'].forEach(id => document.getElementById(id)?.classList.remove('explored'));
        document.getElementById('caseCounter').textContent = `病例 ${currentCaseIndex + 1} / ${unfinishedCases.length}`;
        document.getElementById('prevCaseBtn').style.display = (unfinishedCases.length > 1 && currentCaseIndex > 0) ? 'inline-flex' : 'none';
        document.getElementById('nextCaseBtn').style.display = (unfinishedCases.length > 1 && currentCaseIndex < unfinishedCases.length - 1) ? 'inline-flex' : 'none';
        const otherCheckBtn = document.getElementById('btnOtherCheck');
        otherCheckBtn.style.display = (currentDifficulty === 'basic') ? 'none' : 'block';
        document.getElementById('chiefComplaintCard').scrollIntoView({ behavior: 'smooth', block: 'center' });
    }

    function prevCase() {
        if (currentCaseIndex > 0) { currentCaseIndex--; showCurrentCase(); }
    }
    function nextCase() {
        if (currentCaseIndex < unfinishedCases.length - 1) { currentCaseIndex++; showCurrentCase(); }
    }

    /* ===================== 四诊探索 ===================== */
    function exploreDiag(type) {
        if (!gameStarted || !currentCase) { alert('请先选择关卡。'); return; }
        if (type === 'inquiry') { openInquiryModal(); return; }
        if (type === 'inspection') { openInspectionModal(); return; }
        if (exploredDiags[type]) { alert('该诊法已探索过。'); return; }
        exploredDiags[type] = true; markExplored(type);
        const clue = currentCase.clues[type];
        if (clue) { openSimpleResultModal(clue.displayTitle, clue.displayContent); addClue(type, clue.textSummary); }
    }

    function showOtherCheck() {
        if (!currentCase) return;
        openSimpleResultModal('📋 其他检查', currentCase.otherCheck || '无');
    }
    ...
    function addClue(type, content, tagText) {
        ...
        if (collectedClues.some(c => c.content === content && c.tag === tag)) return;
        collectedClues.push({ tag, tagClass: tagMap[type], content });
        renderClues();
    }

    function renderClues() {
        const area = document.getElementById('clueArea');
        area.innerHTML = collectedClues.map(c => `<div class="clue-item"><span class="clue-tag ${escapeHtml(c.tagClass)}">${escapeHtml(c.tag)}</span><span>${escapeHtmlWithBreaks(c.content)}</span></div>`).join('');
    ...
    function openInquiryModal() {
        ...
        inquiryHistory.forEach(m => appendChat(m.role, m.text));
    ...
    function sendInquiry() {
        ...
        inquiryHistory.push({ role: 'user', text: q }); appendChat('user', q);
        input.value = '';
        ...
        if (best >= 0 && bestScore > 0) {
            ...
            if (!askedInquiryQuestions.includes(best)) {
                askedInquiryQuestions.push(best);
        ...
        setTimeout(() => {
            inquiryHistory.push({ role: 'patient', text: answer }); appendChat('patient', answer);
            if (askedInquiryQuestions.length >= questions.length) { exploredDiags.inquiry = true; markExplored('inquiry'); }
        }, 300);
    }
    function closeInquiryModal() {
        document.getElementById('inquiryModal').style.display = 'none';
        if (currentCase && askedInquiryQuestions.length >= currentCase.clues.inquiry.questions.length) {
            exploredDiags.inquiry = true; markExplored('inquiry');
        }
    }
    ...
    function openInspectionModal() {
        if (!currentCase) return;
        inspectionImages = currentCase.inspectionImages || [tongueImages[currentCase.id] || ''];
        inspectionIndex = 0;
        ...
        if (nonTongue) {
            ...
            if (!inspectionNonTongueAdded) {
                inspectionNonTongueAdded = true;
        ...
        document.getElementById('inspectionImg').onload = function() {
            ...
            document.getElementById('inspectionCounter').textContent = inspectionImages.length > 1 ? (inspectionIndex + 1) + ' / ' + inspectionImages.length : '';
        };
    ...
    document.getElementById('inspectionCounter').textContent = hasMultiple ? (inspectionIndex + 1) + ' / ' + inspectionImages.length : '';
    // 预加载下一张图片
    preloadNextImage();
}
...
function preloadNextImage() {
    ...
    function inspectPrev() { if (inspectionIndex > 0) { inspectionIndex--; renderInspection(); } }
    function inspectNext() { if (inspectionIndex < inspectionImages.length - 1) { inspectionIndex++; renderInspection(); } }
    function closeInspectionModal() {
        document.getElementById('inspectionModal').style.display = 'none';
        // 关闭模态框后清空图片地址，释放内存
        document.getElementById('inspectionImg').src = '';
    }
    function submitTongueJudgment() {
        if (!currentCase) return;
        const userText = document.getElementById('tongueJudgmentInput').value.trim();
        if (!userText) { alert('请填写你的舌象判断。'); return; }
        const tj = currentCase.clues.inspection.tongueJudgment || {};
        const ok = judgeTongueField(userText, tj);
        // 正确答案直接用病例对舌象的原始描述，不自行补充「舌形正常」等原病例未提及的内容
        const correctText = (currentCase.clues.inspection.tongueDesc || '').trim()
            || `舌色${tj.color || '未述'}，舌苔${tj.coating || '未述'}`;
        const clueText = `舌象判断\n${ok ? '✅' : '❌'} 你的描述：${userText}\n正确答案：${correctText}`;
        addClue('inspection', clueText, '望诊·舌象');
        exploredDiags.inspection = true;
    ...
    function submitAnswer() {
        ...
        if (isCorrect) {
            ...
            removeWrongCase(currentCase.id);
        } else {
            if (!dOk && !sOk) {
            ...
            saveWrongCase({ syndrome, disease, basis });
        }
    ...
    function viewAnswer() {
        ...
        markCaseCompleted(currentCase.id);
    }
    function showFullAnalysis(el) {
        const fa = currentCase.fullAnalysis;
        const sourceHtml = currentCase.source ? `<p><strong>病例来源：</strong><span class="source-tag">${escapeHtml(currentCase.source)}</span></p>` : '';
    ...
    function resetCurrentCase() {
        ...
        if (confirm('确定重新探查吗？线索将清除。')) {
            collectedClues = []; exploredDiags = { inspection: false, auscultation: false, inquiry: false, pulse: false };
            inquiryHistory = []; askedInquiryQuestions = []; inspectionNonTongueAdded = false; lastAnswerWasCorrect = false;
            document.getElementById('clueArea').innerHTML = '';
            document.getElementById('answerFeedback').innerHTML = '';
            document.getElementById('fullAnalysisArea').innerHTML = '';
            document.getElementById('inputSyndrome').value = '';
    ...
    function showHistory() {
        if (!currentCase) return;
        openSimpleResultModal('📜 病史（既往史）', currentCase.history || '无');
    }
    ...
    function closeSimpleResultModal() { document.getElementById('simpleResultModal').style.display = 'none'; }

    /* ===================== 病例题库 ===================== */
    function getAllCases() { return (casesDB && casesDB.cases) ? casesDB.cases : []; }
    ...
    async function challengeCaseFromBank(caseId) {
        ...
        const completed = getCompletedCases();
        unfinishedCases = getAllCases().filter(x => x.difficulty === c.difficulty && !completed.includes(x.id));
        if (!unfinishedCases.some(x => x.id === caseId)) unfinishedCases.push(c);
        currentCaseIndex = unfinishedCases.findIndex(x => x.id === caseId);
    ...
    async function rechallengeCase(caseId) {
        ...
        const completed = getCompletedCases();
        unfinishedCases = pool.filter(c => !completed.includes(c.id));
        if (!unfinishedCases.some(c => c.id === caseId)) {
            const c = findCaseById(caseId);
            if (c) unfinishedCases.push(c);
        }
        currentCaseIndex = unfinishedCases.findIndex(c => c.id === caseId);
        if (currentCaseIndex < 0) currentCaseIndex = 0;
        showCurrentCase();
    }
