...
const extra = [
    ...
    ['adv-004', '心悸什么时候发作', /晨起/],
    ['inter-006', '碰水会痛吗', /碰水/],
    // ---- 本轮审计补的口语可达性（四诊归属 + 问诊入口） ----
    // basic-001 闻诊只留声音，痰的色质改由新增的「咳痰情况」承载
    ['basic-001', '有没有咳痰', /黄稠痰/],
    ['basic-001', '会咯痰吗', /黄稠痰/],
    ['basic-001', '痰是什么样的', /黄稠痰/],
    ['basic-001', '痰多不多', /黄稠痰/],
    // 闻诊有「呼吸急促」、答案有「咳甚呼吸急促」，原本问「喘吗」得中性
    ['inter-002', '喘吗', /呼吸急促/],
    ['inter-002', '气短吗', /呼吸急促/],
    // 答案原文「有口苦、咽干」，原本问「咽干吗」得中性
    ['inter-005', '咽干吗', /咽干/],
    // 题面已含「治疗」，答案含服药经过，原本问「治疗过吗」得中性
    ['inter-007', '治疗过吗', /丹参滴丸/],
    // 病名即嗳气，原本问「嗳气吗」得中性
    ['adv-006', '嗳气吗', /嗳气/],
    // 规则词/关键词里的大写写法（CT / MRI / B超）必须能匹配小写化后的输入
    ['inter-005', '做过MRI吗', /MRI/],
    ['adv-002', '做过B超吗', /B超/],
    // 病例主诉写的是「胃脘胀满疼痛」，用户照此发问
    ['inter-004', '胃脘胀痛吗', /胀满疼痛/],
    ['adv-001', '胃脘胀痛吗', /隐痛/],
    ['inter-008', '哪边痛', /颞部/],
];
